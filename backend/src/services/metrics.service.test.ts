...
describe("MetricsService", () => {
  ...
  it("should increment request counter", async () => {
    ...
    expect(metrics).toContain(
      'anchorpoint_requests_total{method="GET",endpoint="/api/test"',
    );
  });
  ...
  it("should record HTTP request with status code", async () => {
    ...
    const metrics = await metricsService.getMetrics();
    expect(metrics).toContain(
      'http_requests_total{method="POST",path="/api/users",status_code="201"',
    );
  });
  ...
  it("should increment error counter", async () => {
    ...
    const metrics = await metricsService.getMetrics();
    expect(metrics).toContain(
      'anchorpoint_errors_total{error_type="ValidationError",endpoint="/api/users"',
    );
  });
