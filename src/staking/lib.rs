...
impl StakingContract {
    ...
    pub fn stake(env: Env, user: Address, amount: i128) {
        ...
        info.lock_end = current_time + env.storage().instance().get::<_, u64>(&DataKey::LockPeriod).unwrap();

        env.storage().persistent().set(&DataKey::Stake(user.clone()), &info);
        env.events().publish((symbol_short!("stake"), user), (amount, info.lock_end));
    }
    ...
    pub fn get_stake_info(env: Env, user: Address) -> StakeInfo {
        env.storage().persistent().get(&DataKey::Stake(user)).unwrap_or(StakeInfo {
            amount: 0,
            last_updated: 0,
