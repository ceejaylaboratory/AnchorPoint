...
impl YieldFarmingDistributor {
    pub fn initialize(env: Env, admin: Address, amm_pool: Address, reward_token: Address, reward_rate: i128) {
        ...
        env.storage().instance().set(&DataKey::LastUpdateLedger, &env.ledger().sequence());
        env.storage().instance().set(&DataKey::RewardPerShareStored, &0i128);
    }
    ...
    pub fn claim_rewards(env: Env, user: Address) -> i128 {
        ...
        let reward: i128 = env.storage().persistent().get(&DataKey::Rewards(user.clone())).unwrap_or(0);
        if reward > 0 {
            env.storage().persistent().set(&DataKey::Rewards(user.clone()), &0i128);
            let reward_token: Address = env.storage().instance().get(&DataKey::RewardToken).unwrap();
            env.invoke_contract::<()>(
                &reward_token,
                &symbol_short!("transfer"),
                soroban_sdk::vec![&env, env.current_contract_address().to_val(), user.to_val(), reward.into()],
            );
        }
        reward
    }
    ...
    pub fn pending_rewards(env: Env, user: Address) -> i128 {
        ...
        let total_shares: i128 = env.invoke_contract(&amm_pool, &soroban_sdk::Symbol::new(&env, "get_total_shares"), soroban_sdk::vec![&env]);
        let user_shares: i128 = env.invoke_contract(&amm_pool, &soroban_sdk::Symbol::new(&env, "get_shares"), soroban_sdk::vec![&env, user.to_val()]);
        
        let last_update: u32 = env.storage().instance().get(&DataKey::LastUpdateLedger).unwrap();
        let current_ledger = env.ledger().sequence();
        let reward_rate: i128 = env.storage().instance().get(&DataKey::RewardRate).unwrap();
        
        let mut reward_per_share: i128 = env.storage().instance().get(&DataKey::RewardPerShareStored).unwrap_or(0);
    ...
    fn _update_user_reward(env: &Env, user: &Address) {
        ...
        let stored: i128 = env.storage().instance().get(&DataKey::RewardPerShareStored).unwrap_or(0);
        let user_paid: i128 = env.storage().persistent().get(&DataKey::UserRewardPerSharePaid(user.clone())).unwrap_or(0);
