...
impl OracleConsumer {
    ...
    pub fn update_price(env: Env, asset: Address) -> PriceData {
        ...
        env.storage().instance().set(&DataKey::PriceRecord(asset.clone()), &price_info);
        
        // Emit an event to notify observers of the price update.
        env.events().publish((symbol_short!("price_upd"), asset), price_info.price);
    ...
    pub fn get_latest_price(env: Env, asset: Address, max_age_seconds: u64) -> i128 {
        let price_info: PriceData = env.storage().instance()
